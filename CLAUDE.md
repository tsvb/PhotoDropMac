...
`photodrop verify <library|manifest.json> [--json] [--xattr]` re-verifies a library (exit `0` all-verified, `1` issues found, `2` no manifest / error), driving the same `VerifyEngine` the app uses. `--xattr` verifies by each file's embedded checksum attribute instead of the manifest (see below), so it works on any folder even after a reorg. `photodrop ingest --from <card> --to <primary> [--archive …] [--preset …] [--[no-]verify] [--[no-]eject] [--description …] [--folder-template …] [--file-template …]` drives `IngestEngine` headlessly. `photodrop heal <library> [--json] [--script <path>]` ([HealEngine.swift](Sources/PhotoDropMac/Core/HealEngine.swift)) is **report-only**: it verifies the library and, for every damaged/missing file, reports whether a healthy copy exists in a recorded mirror — and where. It **never writes to the library**; `--script` emits a reviewable restore script the user runs themselves. Mirror lookup uses the manifest's `destinations` array (all roots written that job; falls back to `[primaryDestination] + archiveDestination` for older manifests).
...
The precision (`yyyyMMdd-HHmmss-SSS`) is load-bearing. `ManifestWriter` writes `.atomic`, which *replaces* an existing file, so two jobs sharing a stamp means the second silently destroys the first's manifest — and losing a manifest is worse than it sounds: `verify` then reports success over whatever records remain, printing `✓ All 1 file …` for a two-file library and exiting 0, rather than failing. At second resolution that collision was reachable in practice (three back-to-back ingests produced two manifests). Milliseconds close it for sequential jobs; two deliberately raced CLI processes could still collide, which would need an `O_EXCL`-style uniquifier. Milliseconds are separated with `-`, not `.`, so the stamp can never be read as a file extension.
